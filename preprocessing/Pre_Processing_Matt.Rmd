...
```{r}
craigslist_file <- 'preprocessing/vehicles.csv' #Car Data
census_median_income_file <- 'preprocessing/census_median_income_2020.csv' #Census Data
census_variable_list_file <- 'preprocessing/census_median_income_variable_list.csv' #Explanation of Census Variables
zcta_mapping_file <- 'preprocessing/zcta_zip_city_map.xlsx' #City to Zipcode Mapping file
```
...
```
In most cases, the description column contains the year of the cars; however, there are 68 rows of data that have NA values in BOTH the year and the description columns. We should drop these rows from the dataset.
...
cars %>% filter(is.na(year))
```
21 - NA datapoints remain for the 'year' column. I don't see the year in the description so we should probably drop these remaining rows from the dataset.
...
There are 17,578 NA values in the 'manufacturer' column. Let's try to pattern match from a list of manufacturers.
```{r}
...
manufacturer_pattern <- 'Acura|Alfa Romeo|Aston Martin|Audi|Bentley|
                      ...
                      Infinity|Isuzu|Jaguar|Jeep|Kia|Land Rover|
                      Lexus|Lincoln|Maserati|Mazda|Mercedes-Benz|Mercury|
                      Mini|Mitsubishi|Nissan|Peugeot|Pontiac|Porsche| 
                      Ram|Saab|Saturn|Scion|Seat|Smart|Subaru|Suzuki|
                      Tesla|Toyota|Volkswagen|Volvo'
                       
```

```{r}
missing_manuf <- cars %>% filter(is.na(manufacturer))
```

```{r}
missing_manuf$matches <- str_extract_all(missing_manuf$description, manufacturer_pattern, simplify = TRUE)[,1]
```
```{r}
missing_manuf %>% filter(matches == '')
```
