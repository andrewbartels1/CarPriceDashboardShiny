...
db_path <- "/Users/matt/Dev/Projects/MGT6203_GroupProject/Database/CraigslistCarsClean.sqlite3"
...
```{r}
cars <- dbGetQuery(conn, "SELECT * FROM vehicles")
```
...
cars <- cars %>% select(id, region, city, state, year, manufacturer, model, 
                        ...
                        transmission, drive, type, paint_color, posting_date, description)
```
...
```
```{r}
zcta_df <- read_xlsx('zcta_zip_city_map.xlsx')
```
...
dbWriteTable(conn, "cars_clean", cars_clean, overwrite = overwrite_table)
