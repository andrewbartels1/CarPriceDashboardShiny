...
```{r}
cars <- merge(cars, census_grouped, by = c("state", "city"))
```
...
Convert 'state', 'manufacturer', 'model' to factor data structure
```{r}
...
```{r}
dbWriteTable(conn, "Ford", ford, overwrite = TRUE)
...
# cars_clean_pm$condition <- as.factor(cars_clean_pm$condition)
cars_sub <- select(cars,year,condition,cylinders,fuel,odometer,price)
glimpse(cars_sub)
